...
while True:
    try:
        ...
        if tweets and tweets.data:
            for tweet in reversed(tweets.data):
                ...
                if tweet_id in processed_mentions:
                    print(f"Skipping tweet {tweet_id} — already processed.")
                    continue
                ...
                try:
                    user_response = get_user(client, author_id)
                    if user_response.data:
                        author = user_response.data.username
                        print(f"✅ Got author: @{author}")
                    else:
                        print(f"⚠️ No user data returned for author ID {author_id}")
                        continue
                except Exception as e:
                    print(f"❌ 409 or other error on author ID {author_id}: {e}")
                    continue

                print(f"Processing tweet {tweet_id} — IN process.")
                read_count += 1
                if f"@{author}" in subs:
        ...
        # my_tweets = get_users_tweets(client, client.get_me().data.id, tweet_fields=["conversation_id"], max_results=5)
        # read_count += 1
